...
impl E1000Driver {
    fn handle_rx_irq(dev: &net::Device, napi: &Napi, data: &NetData) {  
        ...
        if let Some(packets) = recv_packets{

            pkts_compl = packets.len();

            for packet in packets.iter(){
                
                let pkt_len = packet.len();
                let skb = dev.alloc_skb_ip_align(RXBUFFER).unwrap();
                let skb_buffer = unsafe {from_raw_parts_mut(skb.head_data().as_ptr() as *mut u8,pkt_len)};
                
                skb_buffer.copy_from_slice(&packet);
                skb.put(pkt_len as u32);
            ...
            pr_info!("handle_rx_irq has received packets count: {}, bytes count: {}\n",pkts_compl,bytes_compl);
        }
        else{
            pr_warn!("handle_rx_irq has no packets to be received\n");
        }
        data
        .stats
        .rx_bytes
...
impl net::DeviceOperations for E1000Driver {
    ...
    fn open(dev: &Device, data: &NetData) -> Result {
        ...
        // step2 request_irq
        let irq_registration = request_irq(data.irq, irq_data)?;
        // step3 set up irq_handler
        data.irq_handler.store(Box::into_raw(Box::try_new(irq_registration).unwrap()),Ordering::Relaxed);
    ...
    fn start_xmit(
        ...
        if xmit_len==-1{
            pr_warn!("start_xmit failed! E1000 hasn't finished the corresponding previous transmission request\n");
        ...
        dev.completed_queue(1 as u32, skb.len() as u32);
